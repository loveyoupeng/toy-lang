...
public:
  ...
  mlir::ModuleOp generate(ExprAST &ast) {
    ...
    auto mainFunc = mlir::func::FuncOp::create(builder.getUnknownLoc(), "main", funcType);
...
private:
  mlir::Value codegen(ExprAST &node) {
    ...
    if (auto *bin = dynamic_cast<BinaryExprAST *>(&node)) {
      ...
      switch (bin->getOp()) {
      case '+':
        return builder.create<mlir::arith::AddFOp>(builder.getUnknownLoc(), lhs, rhs);
      case '-':
        return builder.create<mlir::arith::SubFOp>(builder.getUnknownLoc(), lhs, rhs);
      case '*':
        return builder.create<mlir::arith::MulFOp>(builder.getUnknownLoc(), lhs, rhs);
      case '/':
        return builder.create<mlir::arith::DivFOp>(builder.getUnknownLoc(), lhs, rhs);
      }
    }
    return nullptr;
  }
